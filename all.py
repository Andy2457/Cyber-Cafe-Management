...
output=c1.fetchall()
l2=tkinter.Label(win,text="Customer Info",fg="black",bg="peru",font=("LingLengLang",40)).grid(row=1, columnspan=4)
cols = ('Customer ID', 'Customer Name', 'Address', 'Mobile No.', 'Email ID', 'Membership', 'Discount')
listBox = ttk.Treeview(win, columns=cols, show='headings')
for col in cols:
    listBox.heading(col, text=col)    
listBox.grid(row=2, column=0, columnspan=2)
for c in output:
    listBox.insert("","end",values=(c[0],c[1],c[2],c[3],c[4],c[5],c[6]))
win.mainloop()
