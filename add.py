...
l2.place(x=80,y=100)
l3=tkinter.Label(win,text="Add New Customers",fg="brown",bg="Peru",font=("The Poster King",30))
    ...
    if c=="":
        ...
        if len(p)==10:
            if m=="Temporary":
                ...
                d="20%"
            c1.execute("insert into Add_new_customer values ("+str(c)+",'"+str(n)+"','"+str(a)+"','"+str(p)+"','"+str(e)+"','"+str(m)+"','"+str(d)+"')")
            conn.commit()
            win.destroy()
...
b1=ttk.Button(win,text="Generate ID:",command=generateid).place(x=600,y=250,width=200,height=40)
...
b3=ttk.Button(win,text='ADD',command=add).place(x=300,y=600,width=100,height=40)
